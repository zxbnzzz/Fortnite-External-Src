...
#include "../controller support.cpp"
...
//	fvector2d screenBottom1 = uengine->worldtscreen(bottom1);
//	fvector2d screenBottom2 = uengine->worldtscreen(bottom2);
//	fvector2d screenBottom3 = uengine->worldtscreen(bottom3);
//	fvector2d screenBottom4 = uengine->worldtscreen(bottom4);
//
//	fvector2d screenTop1 = uengine->worldtscreen(top1);
//	fvector2d screenTop2 = uengine->worldtscreen(top2);
//	fvector2d screenTop3 = uengine->worldtscreen(top3);
//	fvector2d screenTop4 = uengine->worldtscreen(top4);
//
//	// Draw the lines
...
void DrawFilledESP(int x, int y, int w, int h, float color[4], int thickness) {
	SPOOF_FUNC
		...
		/*Vector3 bone0 = uengine->bonewithrotation(skeletal_mesh, 0);
	Vector3 Headpos = uengine->bonewithrotation(skeletal_mesh, 110);*/
	float r = color[0];
	float g = color[1];
	float b = color[2];
	float a = color[3];
	...
	if (esp::box_type == 2) {
     ...
     /*if (esp::box_type == 2)
	{
		 DrawThreeDBox(bone0, Headpos, 100, 100, esp::box_thick, converted_color);
							}*/
		...
		auto ActorLoop() -> void {
			...
			for (int i = 0; i < pointer->player_array_size; ++i) {
				...
				if (esp::rank_esp) {
					...
					uintptr_t habenero = read<uintptr_t>(player_state_enemy + 0xa10); // https://dumpspace.spuckwaffel.com/Games/?hash=6b77eceb&type=classes&idx=AFortPlayerState&member=HabaneroComponent
