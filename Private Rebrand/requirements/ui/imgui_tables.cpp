...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    if (IsItemHovered())
        GetForegroundDrawList()->AddRect(table->OuterRect.Min, table->OuterRect.Max, IM_COL32(255, 165, 0, 255));
    if (IsItemVisible() && table->HoveredColumnBody != -1)
        GetForegroundDrawList()->AddRect(GetItemRectMin(), GetItemRectMax(), IM_COL32(255, 165, 0, 255));
    if (!open)
        return;
    if (table->InstanceCurrent > 0)
    ...
    for (int n = 0; n < table->ColumnsCount; n++)
    {
        ...
        if (IsItemHovered())
        {
            ...
            GetForegroundDrawList()->AddRect(r.Min, r.Max, IM_COL32(255, 165, 0, 255));
        }
    }
