...
struct tab_state
{
    ImVec4 background, shadow, line, text;
    float text_offset;
};
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? c::accent_color : c::checkbox::checkmark_inactive, g.IO.DeltaTime * 6.f);
    ...
    GetWindowDrawList()->AddRect(check_bb.Min, check_bb.Max, GetColorU32(c::checkbox::background_outline), c::checkbox::rounding - 1);

    GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2(it_anim->second.background_offset), check_bb.Max - ImVec2(it_anim->second.background_offset), GetColorU32(it_anim->second.background), c::checkbox::rounding);
    ...
    GetWindowDrawList()->AddText(ImVec2(check_bb.Max.x + style.ItemSpacing.x, check_bb.Min.y), GetColorU32(it_anim->second.text), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    GetWindowDrawList()->AddRect(frame_bb.Min + ImVec2(0, 6), frame_bb.Max - ImVec2(0, 5), GetColorU32(c::slider::background_outline), 100.f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 6), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x, grab_bb.Max.y - 3), GetColorU32(c::accent_color), 100.f);

        GetWindowDrawList()->AddCircleFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 3, grab_bb.Max.y - 5), 5.f, GetColorU32(it_anim->second.circle), 100.f);
    }
